  ...
  return <div className="bulk-modal" role="presentation"><section ref={dialogRef} tabIndex={-1} className="bulk-wizard" role="dialog" aria-modal="true" aria-label="批量造数">
    ...
    <div className="bulk-content"><header className="bulk-header"><div><h1>{STEPS[step - 1]}</h1><p>{['选择目标、来源和保留策略', '选择日期与写入时间', '配置 Tag、Field 和约束', '检查样本并确认写入'][step - 1]}</p></div><button className="bulk-close" type="button" onClick={onClose} aria-label={activeJob && isUnfinishedBulkJob(activeJob.status) ? '最小化批量造数' : '关闭批量造数'} title={activeJob && isUnfinishedBulkJob(activeJob.status) ? '任务会继续在 Bridge 中运行' : '关闭'}>{activeJob && isUnfinishedBulkJob(activeJob.status) ? '—' : '×'}</button></header>
      <div className="bulk-body">
        ...
        {step === 1 && <div className="bulk-section"><div className="bulk-context"><label>连接<select value={connection.id} onChange={event => { const next = connections.find(item => item.id === event.target.value); if (next) onConnectionChange(next) }}>{connections.map(item => { const usable = item.environment === 'test' && !item.readOnly; return <option key={item.id} value={item.id} disabled={!usable}>{item.name}{!usable ? '（仅测试可写）' : ''}</option> })}</select></label><label>Database<select value={database} onChange={event => onDatabaseChange(event.target.value)} disabled={!databases.length}>{databases.map(item => <option key={item}>{item}</option>)}</select></label><small>仅可选择测试可写连接；切换后会重新读取目标表、RP 和 Schema。</small></div><label>目标逻辑前缀<select value={prefix} onChange={event => setPrefix(event.target.value)}>{prefixes.map(item => <option key={item}>{item}</option>)}</select></label><label>Schema 来源（默认最新天表）<select value={sourceMeasurement} onChange={event => setSourceMeasurement(event.target.value)}>{siblings.map(item => <option key={item}>{item}</option>)}</select></label><label>保留策略 RP<select value={retentionPolicy} onChange={event => setRetentionPolicy(event.target.value)}>{policies.map(item => <option key={item.name} value={item.name}>{item.name}{item.isDefault ? '（数据库默认）' : ''} · {duration(item.durationMs)}</option>)}</select></label><div className="bulk-note"><b>结构检查</b><p>{schema ? drift : '正在读取 Schema…'}</p><small>检查来源表与目标天表的 Tag、Field 是否一致。</small></div></div>}
