...
export default function App() {
  ...
  const [tourStatus,setTourStatus]=useState<TourStatus>(()=>initialTourStatus(load(TOUR_STORAGE_KEY,'new'),load('gdb.databaseSwitcherSeen',false)))
  const [tourOpen,setTourOpen]=useState(()=>initialTourStatus(load(TOUR_STORAGE_KEY,'new'),load('gdb.databaseSwitcherSeen',false))==='new')
  ...
  function confirmFavorite(name:string) { const next = [{ id: crypto.randomUUID(), name, sql, database }, ...favorites]; setFavorites(next); save('gdb.favorites', next); setFavoriteDialog(false); toast('已加入收藏') }
...
function FavoriteDialog({database,sql,onClose,onSave}:{database:string;sql:string;onClose:()=>void;onSave:(name:string)=>void}){
  const [name,setName]=useState(`常用命令 · ${database}`)
  const normalizedSql=sql.trim()
  function submit(event:React.FormEvent){event.preventDefault();const value=name.trim();if(value&&normalizedSql)onSave(value)}
  return <div className="modal" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
    <form className="dialog favorite-dialog" role="dialog" aria-modal="true" aria-labelledby="favorite-dialog-title" onSubmit={submit} onKeyDown={event=>{if(event.key==='Escape')onClose()}}>
      <div className="favorite-dialog-head"><div><h2 id="favorite-dialog-title">收藏查询</h2><p>保存当前 SQL，之后可从“收藏”页签快速打开。</p></div><button type="button" className="favorite-dialog-close" onClick={onClose} aria-label="关闭">×</button></div>
      <div className="favorite-dialog-body"><label>收藏名称<input autoFocus value={name} onFocus={event=>event.currentTarget.select()} onChange={event=>setName(event.target.value)} placeholder="输入收藏名称"/></label><div className="favorite-sql-preview"><span>当前 SQL</span><pre>{normalizedSql||'当前查询为空'}</pre></div></div>
      <div className="favorite-dialog-actions"><small><kbd>Enter</kbd> 保存 · <kbd>Esc</kbd> 取消</small><span><button type="button" onClick={onClose}>取消</button><button className="primary" disabled={!name.trim()||!normalizedSql}>保存收藏</button></span></div>
    </form>
  </div>
}
