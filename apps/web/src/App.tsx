...
import { createDiagnosticProvider, migrateClaudeSettings } from './diagnostic-provider'
...
import AgentWorkbench, { type AgentLaunchContext } from './AgentWorkbench'
...
export default function App() {
  ...
  const [primaryWorkspace,setPrimaryWorkspace]=useState<PrimaryWorkspace>(()=>load('gdb.primaryWorkspace','query'))
  const [agentLaunchContext,setAgentLaunchContext]=useState<AgentLaunchContext>()
  ...
  const [view, setView] = useState<ResultView>(()=>visibleResultView(load('gdb.workspace.resultView','result')))
  ...
  const [favoriteDialog, setFavoriteDialog] = useState<Favorite|'new'|null>(null)
  ...
  const [claudeSettings,setClaudeSettings]=useState<ClaudeSettings>(()=>{const defaults:ClaudeSettings={cliPath:'claude',endpoint:'https://api.anthropic.com',model:'claude-sonnet-4-5',maxTokens:2048,fallbackToApi:true};return migrateClaudeSettings({...defaults,...load('gdb.claude.settings',defaults)})})
  ...
  const activeQueryTab = activeTab?.kind==='query' ? activeTab : undefined
  const activeMeasurementDataTab = activeTab?.kind === 'measurement-data' ? activeTab : undefined
  ...
  function openAgentSql(command:string){openQueryTab(command);switchWorkspace('query');toast('已在新查询页签打开')}
  function openAgent(context:AgentLaunchContext){setAgentLaunchContext(context);switchWorkspace('agent')}
  ...
  useEffect(()=>{let active=true;if(!database){setRetentionPolicies([]);return}void bridge.retentionPolicies(database).then(items=>{if(active)setRetentionPolicies(items.map(item=>item.name))}).catch(()=>{if(active)setRetentionPolicies([])});return()=>{active=false}},[database,status])
  ...
  async function refreshSchemaDialog() {
    ...
    const next = await loadSchema(schemaDialog.measurement,true)
    if (next) setSchemaDialog({measurement:schemaDialog.measurement,schema:next})
  }
  ...
  function saveFavorite() { setFavoriteDialog('new') }
  function confirmFavorite(draft:Omit<Favorite,'id'>) {
    const editing=favoriteDialog!=='new'&&favoriteDialog
  ...
  async function exportResult(label:string,extension:string,type:string,content:string) {
    ...
    try {
      const path=await writeExportFile(exportDirectory,filename,content)
      if(path)toast(`${label} 已保存：${path}`)
      else {download(filename,type,content);toast(`${label} 已开始下载`)}
  ...
  return <div className={`app ${sideOpen ? '' : 'sidebar-closed'} ${primaryWorkspace==='agent'?'agent-active':''}`} style={{'--sidebar-width':`${sidebarWidth}px`} as React.CSSProperties}>
    ...
    <header><div className="brand"><span className="brand-mark"/><b>GeminiDB Studio</b></div><nav className="primary-workspaces" aria-label="一级工作区"><button className={primaryWorkspace==='query'?'active':''} onClick={()=>switchWorkspace('query')}>查询与数据</button><button className={primaryWorkspace==='agent'?'active':''} onClick={()=>switchWorkspace('agent')}>Agent 工作台</button>{primaryWorkspace==='query'&&activeQueryTab&&<button onClick={()=>openAgent({sql,error:lastError,database,measurement:selectedTable,schema})}>发送当前查询</button>}</nav><div className="topbar">
...
<div className="side-content">{sideTool === 'connections' ? <section className="side-panel"><PanelTitle title="连接" count={connections.length} action={<button className="panel-add" data-tour="new-connection" onClick={() => setConnectionDialog({...NEW_INFLUX_CONNECTION})}><span>＋</span> 新建</button>}/><div className="panel-scroll connection-list">{!connections.length&&<Empty text="尚未添加连接" sub="点击右上角“新建”开始连接 GeminiDB"/>}{connections.map(connection => <div key={connection.id} className={`connection-item ${connection.id === activeConnection ? 'active' : ''}`}><button className="connection-row" onClick={() => selectConnection(connection)}><span className="connection-glyph"><UiIcon name="connection"/></span><span><b>{connection.name}</b><small>{connection.endpoint}</small></span></button><button className="connection-more" onClick={() => setConnectionDialog(connection)} aria-label={`编辑 ${connection.name}`} title="编辑连接">•••</button></div>)}</div></section> : sideTool === 'catalog' ?
    ...
    {primaryWorkspace==='agent'&&<AgentWorkbench context={{connection:currentConnection,connected:status.includes('已连接'),connections,database,databases,retentionPolicies,onConnectionChange:selectConnection,onDatabaseChange:next=>void changeDatabase(next)}} launchContext={agentLaunchContext} onLaunchConsumed={()=>setAgentLaunchContext(undefined)} onOpenSql={openAgentSql} onNotify={toast}/>}
    ...
    {favoriteDialog&&<FavoriteDialog favorite={favoriteDialog==='new'?undefined:favoriteDialog} database={database} sql={sql} onClose={()=>setFavoriteDialog(null)} onSave={confirmFavorite}/>}
...
function ResultContent({ view, rows, history, favorites, onUseSql, onRestoreSql, onEditFavorite, onRemoveFavorite }: { view: ResultView; rows: QueryRow[]; history: Execution[]; favorites: Favorite[]; onUseSql: (sql: string) => void; onRestoreSql:(sql:string)=>void; onEditFavorite:(favorite:Favorite)=>void; onRemoveFavorite: (id: string) => void }) {
  ...
  if (view === 'favorites') return favorites.length ? <div className="favorite-list">{favorites.map(item => <div className="favorite" key={item.id} onClick={() => onUseSql(item.sql)}><span><b>★ {item.name}</b><code>{item.sql.replace(/\s+/g, ' ')}</code><small>{item.database||'未指定 Database'}</small></span><span className="favorite-actions"><button aria-label={`编辑收藏 ${item.name}`} title="编辑收藏" onClick={e => { e.stopPropagation(); onEditFavorite(item) }}>编辑</button><button aria-label={`删除收藏 ${item.name}`} title="删除收藏" onClick={e => { e.stopPropagation(); onRemoveFavorite(item.id) }}>删除</button></span></div>)}</div> : <Empty text="还没有收藏语句"/>
...
function FavoriteDialog({favorite,database,sql,onClose,onSave}:{favorite?:Favorite;database:string;sql:string;onClose:()=>void;onSave:(favorite:Omit<Favorite,'id'>)=>void}){
  const editing=Boolean(favorite)
  const [name,setName]=useState(favorite?.name||`常用命令 · ${database}`)
  const [draftSql,setDraftSql]=useState(favorite?.sql||sql)
  const [draftDatabase,setDraftDatabase]=useState(favorite?.database||database)
  const normalizedSql=draftSql.trim()
  function submit(event:React.FormEvent){event.preventDefault();const value=name.trim();if(value&&normalizedSql)onSave({name:value,sql:normalizedSql,database:draftDatabase.trim()})}
  return <div className="modal" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
    <form className="dialog favorite-dialog" role="dialog" aria-modal="true" aria-labelledby="favorite-dialog-title" onSubmit={submit} onKeyDown={event=>{if(event.key==='Escape')onClose()}}>
      <div className="favorite-dialog-head"><div><h2 id="favorite-dialog-title">{editing?'编辑收藏':'收藏查询'}</h2><p>{editing?'修改名称、SQL 或所属 Database。':'保存当前 SQL，之后可从“收藏”页签快速打开。'}</p></div><button type="button" className="favorite-dialog-close" onClick={onClose} aria-label="关闭">×</button></div>
      <div className="favorite-dialog-body"><label>收藏名称<input autoFocus value={name} onFocus={event=>event.currentTarget.select()} onChange={event=>setName(event.target.value)} placeholder="输入收藏名称"/></label><label>所属 Database<input value={draftDatabase} onChange={event=>setDraftDatabase(event.target.value)} placeholder="输入 Database"/></label><label>SQL 命令<textarea value={draftSql} onChange={event=>setDraftSql(event.target.value)} placeholder="输入 SQL 命令"/></label></div>
...
function ClaudeSettingsDialog({settings,onClose,onSave}:{settings:ClaudeSettings;onClose:()=>void;onSave:(settings:ClaudeSettings,key:string)=>void}){
  ...
  return <div className="modal"><div className="dialog"><h2>查询诊断设置</h2><p>Claude CLI 优先；不可用时可切换到 Anthropic API 备用。</p><div className="provider-choice"><div className="active"><b>Claude CLI 优先</b><small>使用本机 Claude Code</small></div><label className="provider-fallback"><input type="checkbox" checked={draft.fallbackToApi} onChange={event=>setDraft({...draft,fallbackToApi:event.target.checked})}/><span><b>Anthropic API 备用</b><small>CLI 不可用时自动调用</small></span></label></div><label>Claude 命令路径<input value={draft.cliPath} onChange={event=>setDraft({...draft,cliPath:event.target.value})} placeholder="claude"/></label>{draft.fallbackToApi&&<><label>API 地址<input value={draft.endpoint} onChange={event=>setDraft({...draft,endpoint:event.target.value})}/></label><label>API Key<input type="password" value={key} onChange={event=>setKey(event.target.value)} placeholder="留空表示保留已保存的 Key"/></label></>}<div className="form-row"><label>模型<input value={draft.model} onChange={event=>setDraft({...draft,model:event.target.value})}/></label><label>最大输出<select value={draft.maxTokens} onChange={event=>setDraft({...draft,maxTokens:Number(event.target.value)})}><option>1024</option><option>2048</option><option>4096</option></select></label></div>{testResult&&<p className="setting-result">{testResult}</p>}<div className="privacy-note">查询结果最多 1000 行，可能包含业务数据。API 模式会把结果发送到配置的 Anthropic Endpoint。密码、API Key 和连接凭据不会发送。</div><div className="dialog-actions"><button disabled={testing} onClick={()=>void test()}>{testing?'检测中…':'测试 CLI'}</button><span><button onClick={onClose}>取消</button><button className="primary" onClick={()=>onSave(migrateClaudeSettings(draft),key)}>保存</button></span></div></div></div>
}
