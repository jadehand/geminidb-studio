...
test('app aborts and invalidates validation on context changes and guards execution', () => {
  ...
  assert.match(source, /function invalidateConnectionSession\(\)\s*\{[\s\S]*\+\+connectionSession\.current/)
  assert.match(source, /const sessionGeneration = invalidateConnectionSession\(\)/)
