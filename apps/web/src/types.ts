...
export type QueryWorkspaceTab = { kind:'query'; id:string; name:string; sql:string }
export type MeasurementDataWorkspaceTab = { kind:'measurement-data'; id:string; name:string; connectionId:string; database:string; measurement:string }
...
export type BulkPreview = { previewId:string; expiresAt:number; pointCount:number; maxNewSeries:number; targets:{ date:string; measurement:string; exists:boolean }[]; requiredAcknowledgements:('acknowledgeCreate'|'acknowledgeOverwrite')[]; warnings:BulkIssue[]; samples:{ index:number; lineProtocol:string }[] }
export type BulkJobStatus = { id:string; status:'queued'|'running'|'retrying'|'paused'|'cancelling'|'cancelled'|'succeeded'|'failed'; currentMeasurement:string|null; completedPoints:number; totalPoints:number; completedBatches:number; totalBatches:number; retryCount:number; lastError:BulkIssue|null; startedAt:number; updatedAt:number }
