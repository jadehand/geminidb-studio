...
test('editable field cell supports explicit edit, accept, cancel, and typed controls', async () => {
  ...
  assert.match(source, /acceptFieldInteraction/)
  assert.match(source, /field\.type === 'boolean'/)
  assert.match(source, /<select/)
  assert.match(source, /<input/)
  assert.match(source, /role="alert"/)
  assert.match(source, /onBlur/)
  assert.match(source, /applyEditablePermission/)
  assert.match(source, /tabIndex=\{editable \? 0 : undefined\}/)
})
...
test('measurement view applies edit permissions and updates through the Bridge contract', async () => {
  ...
  assert.match(view, /editable=\{editable && !submitting\}/)
  assert.match(view, /isCurrentSubmission/)
  assert.match(view, /setSubmitting\(false\)/)
  assert.match(view, /role=\{submitStatus\.error \? 'alert' : 'status'\}/)
