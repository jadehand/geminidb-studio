...
function validatedSingleQuery(script) {
  ...
  const uses = statements.map(statement => USE_DATABASE.test(statement))
  const hasQuery = kinds.includes('query')
  const hasWrite = kinds.some(kind => kind === 'insert' || kind === 'write')
  const hasUse = uses.includes(true)
  const hasUnsupported = kinds.some((kind, index) => kind === 'unsupported' && !uses[index])
  if (hasWrite && (hasQuery || hasUse)) throw commandError('MIXED_COMMAND_BATCH', '查询脚本不能包含写命令')
  if (hasUnsupported) throw commandError('UNSUPPORTED_COMMAND', '仅支持单条 InfluxQL 查询')
  ...
  return { statement:statements[0], kind:hasUse ? 'use' : 'query' }
}

export async function executeSingleQuery({ script, executeQuery }) {
  const { statement, kind } = validatedSingleQuery(script)
  if (kind === 'use') return { affectedRows:0, durationMs:0, message:'database 上下文已切换' }
