...
test('all release metadata reports version 0.7.0', async () => {
  ...
  assert.match(cargoToml, /^version = "0\.7\.0"$/m, 'src-tauri/Cargo.toml')
  assert.equal(tauriConfig.version, expectedVersion, 'src-tauri/tauri.conf.json')
  assert.match(bridgeServer, /version:'0\.7\.0'/, 'Bridge /health')
})
